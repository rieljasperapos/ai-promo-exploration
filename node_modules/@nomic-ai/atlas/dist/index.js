...
export class AtlasIndex extends BaseAtlasClass {
    constructor(id, user, options = {}) {
        ...
        this.project =
            options.project ||
                new AtlasDataset(options.project_id, this.viewer);
        this.id = id;
    }
    endpoint() {
    ...
    async atomInformation(ids) {
        ...
        const responseJson = (await this.apiCall(`/v1/project/atoms/get`, 'POST', body));
    ...
    async projections() {
        if (this._projections) {
            ...
            const projections = project_info.atlas_indices.find((d) => d.id === this.id)?.projections ||
                [];
            this._projections = projections.map((d) => new AtlasProjection(d.id, this.viewer, {
                index: this,
    ...
    async nearest_neighbors_by_vector({ k = 10, queries, }) {
        const { neighbors, distances } = (await this.apiCall(`/v1/project/data/get/nearest_neighbors/by_embedding`, 'POST', {
            atlas_index_id: this.id,
            k,
            queries,
        }));
